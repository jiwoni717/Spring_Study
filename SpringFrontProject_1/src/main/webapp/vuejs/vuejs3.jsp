...
<body>
<div class="container">
	<h2 class="text-center">VueJS(디렉티브 : v-for(제어문))</h2>
	<div class="row">
		<table class="table">
			<tr>
				<td>
					<input type=text size=30 class="input-sm" v-model="msg" ref="msg">
					<input type=button class="btn btn-sm btn-default" value="입력" v-on:click="find()">
				</td>
			</tr>
		</table>
		<table class="table">
			...
			<tbody>
				<tr v-for="vo in movie">
					...
					<td class="text-center">
						<img :src="'https://www.kobis.or.kr'+vo.thumbUrl" style="width:30px; height: 40px;">
					</td>
					<td>{{vo.movieNm}}</td>
...
<script>
	new Vue({
		...
		beforeCreate:function(){
			console.log("beforeCreate : Vue객체 생성하기 전...")
		...
		mounted:function(){//**
			...
			// 가상돔과 실제 돔을 비교하여 변경 → diff
			// 가장 많이 사용되는 상태 → 서버에서 데이터를 읽어올 때
		},
		beforeUpdate:function(){
			console.log("beforeUpdate : 상태 변경(데이터값 변경)되기 전...")
			// 멤버 변수 설정 : react → state, Vue → data => data를 다른 메소드에서 변경시에는 반드시 this.
		},
		updated:function(){//**
		...
		destroyed:function(){
			console.log("destroyed : 화면 변경 → Vue객체 해제...")
		},
		// 사용자 정의 함수
		methods:{
			find:function(){
